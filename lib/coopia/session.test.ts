...
import { compactMessages, coopiaEventMetadata, coopiaRequestContext, handoffSummary, isPublicCoopiaPath, officialWhatsAppHandoffUrl, parseCoopiaSession, shouldRecordPageView } from "./session";
...
describe("COOPIA session state", () => {
  ...
  it("uses only a valid official WhatsApp destination", () => { const summary = handoffSummary({ service: "internet" }); expect(officialWhatsAppHandoffUrl("+54 297 123-4567", summary)).toContain("https://wa.me/542971234567?"); expect(officialWhatsAppHandoffUrl(null, summary)).toBeNull(); });
});
