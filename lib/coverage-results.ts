...
export type CoverageRecord = { street_number: number; plan_name: string; technology: string; coverage_status: CoverageStatus };
...
export function selectCoverage(records: CoverageRecord[], requestedNumber: number) {
  ...
  const nearest = records.filter((record) => Math.abs(record.street_number - requestedNumber) === distance)
    .sort((a, b) => a.street_number - b.street_number || a.plan_name.localeCompare(b.plan_name));
