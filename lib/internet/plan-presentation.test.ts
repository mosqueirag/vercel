...
describe("getInternetPlanPresentation", () => {
  it("uses friendly names while keeping commercial values on the source record", () => {
    ...
    expect(presentation).toMatchObject({ displayName: "Plan Comercial Simétrico", technologyLabel: "Fibra óptica", speedLabel: "100 Mbps", secondaryLabel: "100 Mbps de subida" });
  });
  ...
  it("presents the approved ADSL catalog entry with its commercial label", () => {
    expect(getInternetPlanPresentation({ slug: "plan-adsl-5-megas", name: "Plan ADSL 5 Megas", audience: "home", technology: "ADSL", speed_down_mbps: 5, speed_up_mbps: null, installation_price: null, installation_notes: "Internal migration note" })).toMatchObject({ displayName: "Plan ADSL", technologyLabel: "ADSL", speedLabel: "5 Mbps", secondaryLabel: null });
  });
});
