...
export const safePageImageUrl = z.string().trim().max(2000).refine((url) => {
  ...
  try { const parsed = new URL(url); const configuredHost = process.env.NEXT_PUBLIC_SUPABASE_URL ? new URL(process.env.NEXT_PUBLIC_SUPABASE_URL).hostname : ""; return parsed.protocol === "https:" && ((configuredHost !== "" && parsed.hostname === configuredHost && parsed.pathname.startsWith("/storage/v1/object/public/news-images/")) || ((parsed.hostname === "www.coopsar.com.ar" || parsed.hostname === "coopsar.com.ar") && parsed.pathname.startsWith("/wp-content/uploads/"))); } catch { return false; }
