...
export function NewsBrowser({ items }: { items: NewsArticle[] }) {
  ...
  const filtered = useMemo(() => { const needle = normalize(query.trim()); return items.filter((item) => (category === "Todas" || item.category === category) && (!needle || normalize(`${item.title} ${item.excerpt} ${item.category} ${item.body.join(" ")}`).includes(needle))); }, [category, items, query]);
