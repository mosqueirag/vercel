...
const publicEvents = ["journey_started", "page_viewed", "assistant_opened", "human_handoff_requested", "human_handoff_opened", "whatsapp_opened", "payment_portal_opened", "journey_abandoned", "navigation_executed", "contextual_component_rendered", "action_clicked", "quick_access_click", "procedure_selected", "app_download_click", "home_priority_applied", "form_started", "form_completed", "fiber_coverage_check", "fiber_coverage_result", "internet_plans_viewed", "internet_plan_recommended", "plan_view", "plan_selected", "internet_audience_selected", "enterprise_internet_interest", "enterprise_whatsapp_click", "lead_started", "fiber_waitlist_started", "service_request_started", "service_request_submitted", "service_request_status_checked", "complaint_intent_detected", "complaint_route_resolved", "complaint_whatsapp_opened", "help_article_view", "coopia_global_opened", "coopia_global_closed", "coopia_question", "coopia_action_clicked", "coopia_feedback", "coopia_unresolved", "coopia_handoff", "coopia_page_context", "coopia_message_sent", "coopia_intent_detected", "coopia_service_detected", "coopia_action_shown", "coopia_result", "coopia_error"] as const;
...
export function sanitizePublicCoopiaMetadata(metadata: z.infer<typeof schema>["metadata"]) {
  ...
  const { routingWindow, contactPurpose, helpful, ui_type, fallback_type, last_step, page_type, message_length, confidence, orchestration_intent, outcome, plan_id, technology, coverage_status, commercial_availability, source, action_id, procedure_id, resolution_type, priority_action, destination_type, platform, destination, slug, category } = metadata;
  return { ...(routingWindow ? { routingWindow } : {}), ...(contactPurpose ? { contactPurpose } : {}), ...(typeof helpful === "boolean" ? { helpful } : {}), ...(ui_type ? { ui_type } : {}), ...(fallback_type ? { fallback_type } : {}), ...(last_step ? { last_step } : {}), ...(page_type ? { page_type } : {}), ...(typeof message_length === "number" ? { message_length } : {}), ...(typeof confidence === "number" ? { confidence } : {}), ...(orchestration_intent ? { orchestration_intent } : {}), ...(outcome ? { outcome } : {}), ...(plan_id ? { plan_id } : {}), ...(technology ? { technology } : {}), ...(coverage_status ? { coverage_status } : {}), ...(typeof commercial_availability === "boolean" ? { commercial_availability } : {}), ...(source ? { source } : {}), ...(action_id ? { action_id } : {}), ...(procedure_id ? { procedure_id } : {}), ...(resolution_type ? { resolution_type } : {}), ...(priority_action ? { priority_action } : {}), ...(destination_type ? { destination_type } : {}), ...(platform ? { platform } : {}), ...(destination ? { destination } : {}), ...(slug ? { slug } : {}), ...(category ? { category } : {}) };
}
