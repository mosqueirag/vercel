...
const schema = z.object({
  ...
  metadata: z.object({
    ...
    source: z.enum(["enterprise_panel"]).optional(),
  }).optional(),
...
export function sanitizePublicCoopiaMetadata(metadata: z.infer<typeof schema>["metadata"]) {
  ...
  const { routingWindow, contactPurpose, helpful, ui_type, fallback_type, last_step, page_type, message_length, confidence, orchestration_intent, outcome, plan_id, technology, coverage_status, commercial_availability, source } = metadata;
