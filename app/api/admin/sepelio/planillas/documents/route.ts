...
export async function POST(request: Request) {
  ...
  const { error: auditError } = await session.admin.from("funeral_family_update_audit").insert({ request_id: document.request_id, action: "document_viewed", actor_email: session.email });
