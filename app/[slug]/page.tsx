...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const page = servicePages[slug]; if (!page) notFound(); const visual = serviceImages[slug]; return <main><Header /><section className="inner inner-with-photo"><div><span className="tag">{page.eyebrow}</span><h1>{page.title}</h1><p>{page.intro}</p></div>{visual && <div className="inner-photo"><Image src={visual.src} alt={visual.alt} fill sizes="(max-width: 800px) 100vw, 48vw" priority /></div>}</section><section className="section cards">{page.items.map(([title, text, href]) => <article key={title}><h3>{title}</h3><p>{text}</p><Link className="text-link" href={href}>Continuar →</Link></article>)}</section><Contact /><Footer /></main>; }
