...
export function AdminNavigation({ email }: { email: string }) {
  ...
  return <>
    ...
    <header className={styles.topbar}><button className={styles.menuButton} type="button" aria-expanded={open} aria-controls="admin-navigation" onClick={() => setOpen(true)}>Menú</button><span className={styles.topbarTitle}>Centro de Gestión</span><div className={styles.topbarMeta}><span className={styles.staging}>ENTORNO DE PRUEBA · STAGING</span><Link href="/" target="_blank">Ver sitio ↗</Link><span className={styles.user} title="Sesión administrativa">{email.trim().charAt(0).toUpperCase() || "A"}</span></div></header>
