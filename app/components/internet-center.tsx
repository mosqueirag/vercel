...
import { coverageContextMessage, coveragePresentation, coverageTechnologyLabel } from "../../lib/coverage-presentation";
import { consumeInternetJourneyHandoff, saveInternetPlansHandoff, showInternetPlansEvent, type InternetJourneyHandoff, type PublicCoverageResult, type ShowInternetPlansDetail } from "../../lib/internet/coverage-handoff";
...
import { createInternetCoveragePlanDetail, internetCoveragePlanEvent } from "../../lib/internet/coverage-plan-highlight";
...
type PlanInterest = { id: string; slug: string; name: string; speedDownMbps: number | null; speedUpMbps: number | null; priceAmount: number | null; currency: string | null };
...
export function InternetCenter({ variant = "home" }: { variant?: "home" | "page" }) {
  ...
  const [selectedInterest, setSelectedInterest] = useState<PlanInterest | null>(null);
  const displayedPlanCoverage = useRef<string | null>(null);

  useEffect(() => { const controller = new AbortController(); void fetch("/api/public/contacts?service=general", { signal: controller.signal }).then((response) => response.json()).then((data) => { const contact = (data.contacts || []).find((item: { channelType: string; purpose: string }) => item.channelType === "whatsapp" && item.purpose === "general_contact"); if (contact?.value) setWhatsapp(String(contact.value)); }).catch(() => undefined); return () => controller.abort(); }, []);
  ...
  const showCoverage = useCallback((data: CoverageResult, street: string, streetNumber: string, destination: InternetJourneyHandoff["destination"] = "plans") => {
    const preferredPlan = pickCompatiblePlan(data.plans, requestedPlanId);
    setAnswers((current) => ({ ...current, street, streetNumber }));
    setCoverage(data);
    setSelected(preferredPlan);
    setResult(null);
    setStep(destination === "plans" ? 2 : 3);
    window.dispatchEvent(new CustomEvent(internetCoveragePlanEvent, {
      detail: { ...createInternetCoveragePlanDetail(data), preferredPlanId: preferredPlan?.id ?? null },
    }));
  }, [requestedPlanId]);
  ...
  useEffect(() => {
    const onPlanSelection = (event: Event) => {
      const detail = (event as CustomEvent<SelectInternetPlanDetail>).detail;
      if (!detail?.planId) return;
      setRequestedPlanId(detail.planId);
      setSelectedInterest({ id: detail.planId, slug: detail.planSlug, name: detail.planDisplayName || detail.planName || "Plan de Internet", speedDownMbps: detail.speedDownMbps ?? null, speedUpMbps: detail.speedUpMbps ?? null, priceAmount: detail.priceAmount ?? null, currency: detail.currency ?? null });
      setSelected(null);
      setCoverage(null);
      setResult(null);
      setError("");
  ...
  return <section className={`internet-hub ${variant === "page" ? "internet-hub-page" : ""}`} id="contratar">
    ...
    <div className={`recommender recommender-step-${step}`}>
      ...
      <div className="recommender-panel">
        {step === 1 && <form className="question-set" onSubmit={(event) => { event.preventDefault(); void checkCoverage(); }}>{selectedInterest && <div className="coverage-selected-interest"><small>Estás consultando</small><strong>{selectedInterest.name}</strong><span>{selectedInterest.speedDownMbps ? `${selectedInterest.speedDownMbps} Mbps` : "Internet"}{selectedInterest.priceAmount !== null ? ` · ${new Intl.NumberFormat("es-AR", { style: "currency", currency: selectedInterest.currency || "ARS", maximumFractionDigits: 0 }).format(selectedInterest.priceAmount)} por mes` : ""}</span><em>Oferta de referencia</em></div>}<div className="field-row"><label>Calle<input required value={answers.street} onChange={(e) => setAnswers({ ...answers, street: e.target.value })} placeholder="Ej.: San Martín" /></label><label>Altura<input required inputMode="numeric" value={answers.streetNumber} onChange={(e) => setAnswers({ ...answers, streetNumber: e.target.value.replace(/\D/g, "").slice(0, 6) })} placeholder="Ej.: 1250" /></label></div>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary" disabled={checkingCoverage}>{checkingCoverage ? "Consultando disponibilidad…" : "Consultar cobertura"}</button></form>}
        {step === 2 && <div className="recommendation">{(() => {
          const presentation = coveragePresentation(coverage);
          const hasPlans = hasPublishedCompatiblePlans(coverage);
          const preferredPlanIsCompatible = requestedPlanIsCompatible(coverage?.plans ?? [], requestedPlanId);
          const validationRequired = coverage?.nextAction === "coverage_validation";
          const interestPrice = selectedInterest?.priceAmount === null || selectedInterest?.priceAmount === undefined ? null : new Intl.NumberFormat("es-AR", { style: "currency", currency: selectedInterest.currency || "ARS", maximumFractionDigits: 0 }).format(selectedInterest.priceAmount);
          const showPlanLink = hasPlans || (coverage?.technologies.length ?? 0) > 0;
          const openOptions = () => {
            if (!coverage) return;
            track("internet_options_opened", coverage.technology || coverage.coverageStatus);
            if (variant === "home") {
              saveInternetPlansHandoff(sessionStorage, { street: answers.street, number: answers.streetNumber, coverage });
              router.push("/internet#planes");
              return;
            }
            const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
            document.querySelector("#planes")?.scrollIntoView({ behavior: reduceMotion ? "auto" : "smooth", block: "start" });
          };
          return <>{presentation.eyebrow && <small>{presentation.eyebrow}</small>}<h4>{presentation.title}</h4>{coverage && <section className={`coverage-result ${coverage.coverageStatus}`} aria-live="polite"><p>{coverageContextMessage(coverage, coverage.message)}</p>{presentation.showTechnologies && <div className="coverage-technologies" aria-label="Tecnologías disponibles">{coverage.technologies.map((technology) => <span key={technology}>{coverageTechnologyLabel(technology)}</span>)}</div>}{hasPlans ? <div className="compatible-plans"><strong>Opciones disponibles para tu domicilio</strong>{!preferredPlanIsCompatible && <p className="coverage-plan-note">Para este domicilio te mostramos otras opciones compatibles.</p>}{coverage.plans.map((plan) => <button type="button" className={selected?.id === plan.id ? "selected-plan" : ""} key={plan.id} onClick={() => { setSelected(plan); track("plan_selected", plan.slug); }}><strong>{getInternetPlanPresentation({ ...plan, audience: "", installation_price: null, installation_notes: null }).displayName}</strong>{plan.technology && <span>{coverageTechnologyLabel(plan.technology)}</span>}{plan.speed_down_mbps && <span>{plan.speed_down_mbps} Mbps</span>}{plan.price_amount !== null && <span>{new Intl.NumberFormat("es-AR", { style: "currency", currency: plan.currency || "ARS", maximumFractionDigits: 0 }).format(plan.price_amount)}</span>}</button>)}</div> : validationRequired && selectedInterest ? <div className="coverage-plan-interest"><small>Oferta de referencia</small><strong>{selectedInterest.name}</strong><span>{selectedInterest.speedDownMbps ? `${selectedInterest.speedDownMbps} Mbps` : "Internet"}{interestPrice ? ` · ${interestPrice} por mes` : ""}</span><em>Dato comercial a validar.</em></div> : !validationRequired && <p>Podemos continuar con la validación técnica o el contacto comercial.</p>}</section>}<div className="recommendation-actions"><button onClick={() => setStep(1)}>Modificar domicilio</button>{showPlanLink && <button onClick={openOptions}>Ver opciones</button>}<button className="primary" onClick={() => { if (requestType() === "fiber_waitlist") track("fiber_waitlist_started"); setStep(3); }}>{operationLabel}</button>{validationRequired && whatsapp && <a className="coverage-whatsapp" onClick={() => track("whatsapp_opened", "internet_validation")} href={`https://wa.me/${whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent("Hola, quiero consultar por Internet.")}`}>Hablar con COOPSAR</a>}</div></>;
        })()}</div>}
        {step === 3 && !result && <form className="lead-form" onSubmit={submit}><h4>{requestType() === "fiber_waitlist" ? "Datos para avisarte" : "Datos para contactarte"}</h4><label>El servicio es para<select value={answers.type} onChange={(e) => setAnswers({ ...answers, type: e.target.value as Answers["type"] })}><option value="hogar">Hogar</option><option value="comercio">Comercio</option><option value="empresa">Empresa</option></select></label><div className="field-row"><label>Nombre y apellido<input required name="name" minLength={3} /></label><label>Teléfono<input required name="phone" type="tel" minLength={8} /></label></div><div className="field-row"><label>Correo electrónico (opcional)<input name="email" type="email" /></label><label>Domicilio<input required name="address" minLength={5} defaultValue={`${answers.street} ${answers.streetNumber}`.trim()} /></label></div><label>Barrio o zona (opcional)<input name="zone" /></label><label className="check"><input required name="consent" type="checkbox" /> Autorizo a COOPSAR a contactarme por esta solicitud.</label><label className="check"><input name="marketingOptIn" type="checkbox" /> Quiero recibir novedades de Internet.</label>{error && <p className="form-error">{error}</p>}<button className="primary">Revisar y confirmar solicitud</button></form>}
        {result && <div className={result.stored ? "success-card" : "pending-card"}><strong>{result.stored ? "Solicitud recibida" : "Configuración pendiente"}</strong><p>{result.message}</p><b>{result.requestNumber}</b>{whatsapp ? <a onClick={() => track("whatsapp_opened", "internet_validation")} href={`https://wa.me/${whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola, quiero consultar por internet. Solicitud ${result.requestNumber}.`)}`}>Hablar con COOPSAR →</a> : <p>El canal de contacto se publicará cuando COOPSAR lo confirme.</p>}</div>}
      </div>
    </div>
  </section>;
}
