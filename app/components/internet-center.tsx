...
export function InternetCenter({ variant = "home" }: { variant?: "home" | "page" }) {
  ...
  return <section className={`internet-hub ${variant === "page" ? "internet-hub-page" : ""}`} id="contratar">
    ...
    <div className="recommender">
      <div className="recommender-copy"><span className="eyebrow eyebrow-light">Disponibilidad por domicilio</span><h3>{variant === "page" ? "¿Qué alternativa llega a tu casa?" : "Consultá el plan disponible en tu domicilio"}</h3><p>{variant === "page" ? "Ingresá calle y altura. Te mostramos la tecnología y el próximo paso con información oficial." : "Buscamos servicios activos en la dirección indicada o en alturas cercanas de la misma calle. La factibilidad final requiere validación técnica."}</p>{variant === "home" && <div className="step-indicator"><span className={step >= 1 ? "active" : ""}>1</span><i /><span className={step >= 2 ? "active" : ""}>2</span><i /><span className={step >= 3 ? "active" : ""}>3</span></div>}</div>
