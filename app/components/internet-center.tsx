...
function coveragePresentation(coverage: CoverageResult | null) {
  const isZone = coverage?.coverageSource === "geographic_zone";
  const isFtthZone = isZone && coverage?.technologies.length === 1 && coverage.technologies[0] === "FTTH";
  const isAdslZone = isZone && coverage?.technologies.length === 1 && coverage.technologies[0] === "ADSL";
  return {
    isZone,
    eyebrow: isFtthZone ? "Zona de Fibra Óptica" : isZone ? "Zona habilitada — sujeto a validación técnica" : coverage?.coverageStatus === "available" ? "Información de cobertura" : "Validación requerida",
    title: isFtthZone ? "Fibra Óptica disponible en tu zona" : isAdslZone ? "ADSL disponible en tu zona" : isZone ? "Internet disponible en tu zona" : coverage?.commercialAvailability ? "Planes compatibles" : coverage?.coverageStatus === "nearby" ? "Validación técnica requerida" : "Sin cobertura confirmada",
  };
}
