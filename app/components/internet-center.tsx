  ...
  return <section className="internet-hub" id="internet">
    ...
    <div className="recommender" id="contratar">
      ...
      <div className="recommender-panel">
        ...
        {step === 2 && <div className="recommendation"><small>{coverage?.status === "exact" ? "Domicilio encontrado" : coverage?.status === "probable" ? "Coincidencia por cercanía" : "Resultado del padrón"}</small><h4>{coverage?.service ? "Plan disponible" : "No pudimos identificar un plan"}</h4>{coverage && <section className={`coverage-result ${coverage.status}`} aria-live="polite">{coverage.service && <strong>{coverage.service.planName}</strong>}<p>{coverage.message}</p></section>}<p>El resultado se obtiene exclusivamente del padrón de servicios cargado. No confirma disponibilidad para una instalación nueva hasta completar la validación técnica.</p><div><button onClick={() => setStep(1)}>Modificar domicilio</button><button className="primary" onClick={() => setStep(3)}>Solicitar validación</button></div></div>}
