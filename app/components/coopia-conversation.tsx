...
export function CoopiaConversation({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={compact ? "coopia-conversation coopia-compact" : "coopia-conversation"}>
    <form className="chat-input" onSubmit={submit}><label className="sr-only" htmlFor={compact ? "coopia-global-query" : "assistant-query"}>Tu consulta</label><textarea id={compact ? "coopia-global-query" : "assistant-query"} value={coopia.input} maxLength={1200} onChange={(event) => coopia.setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={compact ? "¿En qué puedo ayudarte?" : "¿Qué necesitás resolver hoy?\nPodés contarme con tus palabras."} rows={compact ? 2 : 3} /><button disabled={!coopia.input.trim() || coopia.loading} aria-label="Enviar consulta">Enviar <span>↑</span></button></form><div className="suggestions coopia-quick-actions" aria-label="Consultas sugeridas">{suggestions.map((item) => <button type="button" key={item.prompt} onClick={() => void coopia.ask(item.prompt)}>{item.label}</button>)}</div>
    {(hasReply || coopia.aiLimited || coopia.error || coopia.loading) && <div className="chat-log" aria-live="polite">
      ...
      {coopia.aiLimited && <div className="limit-card"><strong>Seguimos con tu gestión</strong><p>La asistencia con IA está momentáneamente limitada, pero COOPIA sigue disponible para pagos, cobertura, problemas de servicios, trámites y atención personal.</p><div><a href={coopia.handoffUrl} onClick={() => coopia.track("coopia_handoff", undefined, "whatsapp")}>Hablar con una persona ↗</a></div></div>}
