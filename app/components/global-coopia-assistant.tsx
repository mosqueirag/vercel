...
export function GlobalCoopiaAssistant() {
  ...
  return <div className="global-coopia"><button ref={triggerRef} type="button" className="global-coopia-trigger" aria-expanded={coopia.isOpen} aria-controls="global-coopia-panel" onClick={() => coopia.setOpen(!coopia.isOpen)}><span>✦</span><strong>Preguntale a COOPIA</strong><small>Asistencia durante tu gestión</small></button>{coopia.isOpen && <aside ref={panelRef} id="global-coopia-panel" className="global-coopia-panel" role="dialog" aria-modal="true" aria-label="Asistente COOPIA"><header><div><span className="assistant-avatar">✦</span><div><strong>COOPIA</strong><small>Asistencia durante tu gestión</small></div></div><button type="button" aria-label="Cerrar COOPIA" onClick={() => coopia.setOpen(false)}>×</button></header><CoopiaConversation compact /></aside>}</div>;
}
