...
export function CoopiaProvider({ children }: { children: ReactNode }) {
  ...
  const [messages, setMessages] = useState<CoopiaMessage[]>([]), [input, setInput] = useState(""), [loading, setLoading] = useState(false), [error, setError] = useState(""), [aiLimited, setAiLimited] = useState(false), [assistantResult, setAssistantResult] = useState<AssistantResult | null>(null), [assistantResultKey, setAssistantResultKey] = useState(""), [isOpen, setOpenState] = useState(false), [ids, setIds] = useState({ journeyId: "", sessionId: "" });
  ...
  const recordShownActions = useCallback((result: AssistantResult, resultKey: string, visibleActionIds: AssistantResult["recommendedActions"][number]["id"][]) => {
    if (!ids.journeyId || !resultKey || visibleActionIds.length === 0) return;
    for (const event of takeShownActionEvents({ journeyId: ids.journeyId, resultKey, result, visibleActionIds, seen: shownActionFingerprints.current })) {
      track("coopia_action_shown", event.metadata, event.action, event.result, undefined, event.context);
    }
  }, [ids.journeyId, track]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const next = stored || { messages: [], aiLimited: false, journeyId: sessionStorage.getItem("coopsar-journey-id") || createJourneyId(), sessionId: sessionStorage.getItem("coopsar-session-id") || createSessionId() };
      sessionStorage.setItem("coopsar-journey-id", next.journeyId); sessionStorage.setItem("coopsar-session-id", next.sessionId);
      setMessages(next.messages); setAiLimited(next.aiLimited); setIds({ journeyId: next.journeyId, sessionId: next.sessionId });
  ...
  useEffect(() => { if (ids.journeyId) sessionStorage.setItem(coopiaSessionStorageKey, JSON.stringify({ messages: compactMessages(messages), aiLimited, ...ids, intent: navigation.intent, service: navigation.service })); }, [aiLimited, ids, messages, navigation.intent, navigation.service]);
  useEffect(() => {
