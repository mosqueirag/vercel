...
function normalizedCopy(value: string) { return value.toLocaleLowerCase("es-AR").replace(/[^a-záéíóúüñ0-9]+/g, " ").trim(); }
function addsNewInformation(message: string, title: string) { const normalizedMessage = normalizedCopy(message); const normalizedTitle = normalizedCopy(title); return Boolean(normalizedMessage) && normalizedMessage !== normalizedTitle && !normalizedMessage.includes(normalizedTitle) && !normalizedTitle.includes(normalizedMessage); }
function SimpleCard({ result }: { result: AssistantResult }) { const incident = ["outage", "partial", "maintenance"].includes(String(result.ui?.data.status)); const payment = result.ui?.type === "payment"; const actions = coopiaStepActions(result.actions); const title = payment ? "Pagá tu factura" : incident ? "Incidencia informada" : "Sin incidencia general confirmada"; const showMessage = !payment && addsNewInformation(result.message, title); return <section className={`assistant-card simple-card ${payment ? "payment-card" : "status-card"}`}>{!payment && <div className="assistant-status-icon">{incident ? "!" : "i"}</div>}<div>{!payment && <small>{`Estado de ${String(result.ui?.data.service || "servicio")}`}</small>}<h3>{title}</h3>{showMessage && <p>{result.message}</p>}<div className="assistant-card-actions">{actions.map((action, index) => <ActionLink action={action} primary={index === 0} complaintRoute={result.complaintRoute} resultContext={result} key={action.id} />)}</div></div></section>; }
function ComplaintServicePicker({ onSelect }: { onSelect: (service: string) => void }) { return <section className="assistant-card"><small>Reclamos</small><h3>Elegí el servicio</h3><div className="assistant-card-actions">{["Energía", "Internet", "Telefonía", "Sepelio"].map((service) => <button type="button" className="assistant-action" key={service} onClick={() => onSelect(service)}>{service}</button>)}</div></section>; }
...
function GenericActionsCard({ result }: { result: AssistantResult }) {
  ...
  const title = serviceTitles[result.service];
  return <section className="assistant-card generic-actions-card"><h3>{title}</h3>{addsNewInformation(result.message, title) && <p>{result.message}</p>}<div className="assistant-card-actions">{actions.map((action, index) => <ActionLink action={action} primary={index === 0} complaintRoute={result.complaintRoute} resultContext={result} key={action.id} />)}</div></section>;
}
