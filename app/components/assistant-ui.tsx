...
function FiberCoverageCard({ result }: { result: AssistantResult }) {
  ...
  const action = coverage?.plans.length ? "Ver planes" : coverage?.nextAction === "fiber_waitlist" ? "Solicitar aviso de cobertura" : coverage?.nextAction === "coverage_validation" ? "Solicitar validación técnica" : "Continuar con la solicitud";
  const presentation = coveragePresentation(coverage);
  return <section className="assistant-card coverage-card"><header><h3>¿Dónde querés instalar Internet?</h3></header>{!coverage ? <form onSubmit={submit}><label>Calle<input value={street} minLength={3} required onChange={(event) => setStreet(event.target.value)} placeholder="Ej.: San Martín" /></label><label>Altura<input value={number} required inputMode="numeric" onChange={(event) => setNumber(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="Ej.: 1250" /></label><button disabled={loading}>{loading ? "Consultando…" : "Consultar cobertura"}</button>{error && <p role="alert" className="form-error">{error}</p>}</form> : <div className={`assistant-result ${coverage.coverageStatus}`}><h4>{presentation.title}</h4><p>{coverage.message}</p>{coverage.plans.map((plan) => <div className="assistant-plan" key={plan.id}><strong>{plan.name}</strong>{plan.technology && <span>{plan.technology}</span>}{plan.speed_down_mbps && <span>{plan.speed_down_mbps} Mbps</span>}{plan.price_amount !== null && <span>{new Intl.NumberFormat("es-AR", { style: "currency", currency: plan.currency || "ARS", maximumFractionDigits: 0 }).format(plan.price_amount)}</span>}</div>)}<div><a className="assistant-action assistant-action-primary" href={coverage.plans.length ? "#planes" : "#contratar"} onClick={() => { track(nav.journeyId, nav.sessionId, coverage.nextAction === "fiber_waitlist" ? "fiber_waitlist_started" : "navigation_executed", coverage.plans.length ? "SHOW_INTERNET_PLANS" : "REQUEST_INSTALLATION", undefined, undefined, result); document.querySelector(coverage.plans.length ? "#planes" : "#contratar")?.scrollIntoView({ behavior: "smooth" }); }}>{action} →</a><button type="button" onClick={() => setCoverage(null)}>Modificar domicilio</button></div></div>}</section>;
}
