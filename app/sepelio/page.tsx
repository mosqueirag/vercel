...
export const metadata: Metadata = { title: "Servicio de Sepelio | COOPSAR", description: "Orientación y acceso directo a la guardia del Servicio Solidario de Sepelios de COOPSAR." };
...
export default async function SepelioPage() {
  ...
  return <><Header /><main className="sepelio-page">
    ...
    {candidates.length > 0 && <section className="sepelio-candidates section-shell" aria-label="Información del servicio en revisión"><p className="eyebrow">Entorno de prueba · contenido en revisión</p><h2>Información del servicio</h2><p className="sepelio-candidate-notice">Este contenido histórico está visible sólo para revisión interna. Confirmá las condiciones vigentes con COOPSAR.</p><div>{candidates.map((candidate) => <article key={candidate.title}><h3>{candidate.title}</h3><p>{candidate.body}</p><a href={candidate.sourceUrl} target="_blank" rel="noreferrer">Ver fuente histórica ↗</a></article>)}</div></section>}
    <section className="sepelio-guidance section-shell" aria-labelledby="sepelio-guidance-title"><div><p className="eyebrow">Orientación</p><h2 id="sepelio-guidance-title">Información clara, sin reemplazar la atención humana.</h2></div><div><p>Las consultas urgentes continúan por guardia. Para información no urgente o una gestión específica, elegí el canal adecuado.</p><SepelioCoopiaAction className="text-link" prompt="Quiero información sobre el servicio de sepelio.">Abrir COOPIA</SepelioCoopiaAction></div></section>
    ...
    <section className="sepelio-closing section-shell"><h2>Estamos para orientarte.</h2><p>Elegí el canal que mejor se adapte a lo que necesitás.</p><div><a className="button sepelio-primary-action" href={guard.href}>Llamar a guardia <span>→</span></a><SepelioCoopiaAction className="button sepelio-secondary-action" prompt="Necesito orientación sobre el servicio de sepelio.">Abrir COOPIA</SepelioCoopiaAction></div></section>
  </main><Footer /></>;
}
