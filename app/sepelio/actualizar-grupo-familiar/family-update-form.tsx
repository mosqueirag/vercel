...
export function FamilyUpdateForm({ guardHref }: { guardHref: string }) {
  ...
  async function submit() {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "No pudimos registrar la solicitud."); } finally { setBusy(false); }
  }
  ...
  return <form className={styles.form} noValidate onSubmit={(event) => { event.preventDefault(); if (step < 3) { if (canContinue) setStep((current) => current + 1); return; } if (canContinue) void submit(); }}>
    <ol className={styles.steps} aria-label="Pasos del trámite"><li className={step >= 1 ? styles.active : ""}>1. Titular</li><li className={step >= 2 ? styles.active : ""}>2. Grupo familiar</li><li className={step >= 3 ? styles.active : ""}>3. Confirmación</li></ol>
    {step === 1 && <fieldset><legend>Datos del titular</legend><p>Usaremos estos datos sólo para revisar esta solicitud.</p><div className={styles.grid}>
      <label>Número de asociado o referencia<input required value={holder.memberNumber} onChange={(event) => setHolder({ ...holder, memberNumber: event.target.value })} /></label>
      <label>Nombre y apellido<input required autoComplete="name" value={holder.holderFullName} onChange={(event) => setHolder({ ...holder, holderFullName: event.target.value })} /></label>
      <label>DNI<input required inputMode="numeric" pattern="[0-9]{7,8}" value={holder.holderDni} onChange={(event) => setHolder({ ...holder, holderDni: event.target.value.replace(/\D/g, "") })} /></label>
      <label>Teléfono<input required inputMode="tel" autoComplete="tel" value={holder.phone} onChange={(event) => setHolder({ ...holder, phone: event.target.value })} /></label>
      <label className={styles.wide}>Correo electrónico <span>(opcional)</span><input type="email" autoComplete="email" value={holder.email} onChange={(event) => setHolder({ ...holder, email: event.target.value })} /></label>
    </div></fieldset>}
    ...
    {step === 3 && <fieldset><legend>Revisá antes de enviar</legend><div className={styles.summary}><p><strong>Titular:</strong> {holder.holderFullName}</p><p><strong>Integrantes informados:</strong> {members.length}</p><p>La solicitud será revisada por el equipo de COOPSAR. No confirma modificaciones ni cobertura hasta completar la validación correspondiente.</p></div><label className={styles.consent}><input required type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /> Autorizo a COOPSAR a usar estos datos únicamente para gestionar esta solicitud.</label></fieldset>}
    ...
    <div className={styles.actions}>{step > 1 && <button type="button" className={styles.back} onClick={() => setStep((current) => current - 1)}>Volver</button>}<button className={styles.primaryAction} disabled={!canContinue || busy} type="submit">{busy ? "Enviando…" : step === 3 ? "Enviar solicitud" : "Continuar"}</button></div>
    <p className={styles.help}>¿Es una situación urgente? <a href={guardHref}>Llamá a la guardia de sepelio</a>.</p>
  </form>;
}
