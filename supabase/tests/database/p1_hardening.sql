...
select lives_ok($$ select public.create_internet_request_v2_with_outbox('NET-2026-DDDDDDDD','JRN-2026-ABCDEF12','SES-ABCDEF1234567890','fiber_waitlist','hogar','Persona Test','2974000000',null,'CALLE TEST',500,'', 'unknown', null, null, true, false, 'test', repeat('e',64)) $$, 'server atomically creates a fiber waitlist and its outbox event');
select is((select status from public.internet_requests where request_number='NET-2026-DDDDDDDD'), 'waiting_coverage', 'waitlist receives the private waiting status');
select is((select count(*)::integer from public.integration_outbox where payload->>'requestNumber'='NET-2026-DDDDDDDD' and event_type='fiber_waitlist.created'), 1, 'waitlist creates one minimal outbox event');
